  ...
  return (
    <div className="min-h-screen bg-[#F9F5F2]">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Breadcrumb */}
        <div className="mb-6 text-xs font-elegant tracking-widest uppercase text-gray-400">
          <Breadcrumb items={[{ label: 'Bag', href: '/cart' }, { label: 'Checkout' }]} />
        </div>

        {/* Header */}
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-3xl font-fairplay text-[#1C1C1C]">Checkout</h1>
        ...
        <div className="flex items-center justify-center mb-10">
          {steps.map((s, i) => (
            <div key={s.id} className="flex items-center">
              <div className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-elegant tracking-widest uppercase transition-all ${i <= stepIndex ? 'text-white' : 'text-gray-400 bg-gray-100'}`}
                style={i <= stepIndex ? { backgroundColor: '#B76E79' } : {}}>
                {s.icon}
                <span className="hidden sm:inline">{s.label}</span>
              </div>
              {i < steps.length - 1 && (
                <div className={`w-12 h-px mx-1 transition-colors ${i < stepIndex ? '' : 'bg-gray-200'}`}
                  style={i < stepIndex ? { backgroundColor: '#B76E79' } : {}} />
              )}
            </div>
          ))}
