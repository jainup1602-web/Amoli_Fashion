...
export default function WishlistPage() {
  ...
  if (wishlistItems.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center py-20 px-4" style={{ backgroundColor: '#FDFCF0' }}>
        <div className="text-center p-16 border border-gold/20 shadow-sm max-w-lg w-full" style={{ backgroundColor: '#FDFCF0' }}>
          <Heart className="h-16 w-16 mx-auto mb-6" strokeWidth={1} style={{ color: '#1A1A1A' }} />
          <h2 className="text-3xl font-playfair text-[#1C1C1C] mb-4">Your Wishlist is Empty</h2>
  ...
  return (
    <div className="min-h-screen py-16" style={{ backgroundColor: '#FDFCF0' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <span className="text-gold tracking-[0.2em] text-sm uppercase mb-4 block font-elegant">Your Favorites</span>
          <h1 className="text-4xl md:text-5xl font-playfair text-[#1C1C1C]">Wishlist</h1>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-6">
          {wishlistItems.map((item) => (
            <div key={`wishlist-item-${item.productId}`} className="group border border-gold/10 overflow-hidden transition-luxury hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:border-gold/30" style={{ backgroundColor: '#FDFCF0' }}>
              <Link href={`/products/${item.slug}`}>
                <div className="relative aspect-square" style={{ backgroundColor: '#FDFCF0' }}>
                  {/* Primary image */}
                  <Image
                    src={item.image}
                    alt={item.name}
              ...
              <div className="p-2 sm:p-4">
                <Link href={`/products/${item.slug}`}>
                  <h3 className="font-playfair text-base text-[#1C1C1C] line-clamp-2 hover:text-gold transition-colors mb-3 min-h-[2.5rem] leading-snug">
                    {item.name}
                  </h3>
                </Link>
